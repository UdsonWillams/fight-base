...
from app.database.models.base import Fighter, Event, Fight, User
...
class UFCDatasetImporter:
    ...
    def import_fighters(self, csv_path: str, system_user: User):
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    ...
                    existing = self.session.query(Fighter).filter_by(
                        ufcstats_id=ufcstats_id
                    ).first()
                    
                    # Converter altura e alcance de cm para inches e cm
                    height_cm = self.safe_float(row.get('height'))
                    reach_cm = self.safe_float(row.get('reach'))
                    height_inches = height_cm / 2.54 if height_cm else None
                    reach_inches = reach_cm / 2.54 if reach_cm else None
                    ...
                    if (self.stats['fighters_created'] + self.stats['fighters_updated']) % 100 == 0:
                        print(f"  ⏳ Processados {self.stats['fighters_created'] + self.stats['fighters_updated']} lutadores...")
                        self.session.commit()
        ...
        print(f"✓ Lutadores importados: {self.stats['fighters_created']} criados, {self.stats['fighters_updated']} atualizados")
    ...
    def import_events(self, csv_path: str, system_user: User):
        ...
        for event_id, event_data in events_data.items():
            try:
                existing = self.session.query(Event).filter_by(
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                try:
                    ...
                    # Verificar se já existe
                    existing = self.session.query(Fight).filter_by(
                    ...
                    if r_id not in self.fighter_id_map or b_id not in self.fighter_id_map:
                        continue
                    ...
                    fight_data = {
                        ...
                        'r_sig_str_attempted': self.safe_int(row.get('r_sig_str_atmpted')),
                        'r_total_str_landed': self.safe_int(row.get('r_total_str_landed')),
                        'r_total_str_attempted': self.safe_int(row.get('r_total_str_atmpted')),
                        ...
                        'r_ctrl_seconds': self.parse_time_to_seconds(row.get('r_ctrl', '')),
                        # Estatísticas Blue Corner (fighter2)
                        'b_kd': self.safe_int(row.get('b_kd')),
                        'b_sig_str_landed': self.safe_int(row.get('b_sig_str_landed')),
                        'b_sig_str_attempted': self.safe_int(row.get('b_sig_str_atmpted')),
                        'b_total_str_landed': self.safe_int(row.get('b_total_str_landed')),
                        'b_total_str_attempted': self.safe_int(row.get('b_total_str_atmpted')),
                        ...
                        'b_ctrl_seconds': self.parse_time_to_seconds(row.get('b_ctrl', '')),
                        'created_by': 'import_script',
                        'updated_by': 'import_script'
                    }
                    ...
                    if self.stats['fights_created'] % 100 == 0:
                        print(f"  ⏳ Processadas {self.stats['fights_created']} lutas...")
                        self.session.commit()
    ...
    def update_fighter_cartels(self):
        ...
        fighters = self.session.query(Fighter).filter(Fighter.ufcstats_id.isnot(None)).all()
        ...
        for fighter in fighters:
            try:
                ...
                ).order_by(Fight.created_at.desc()).all()
