...
def example_queries():
    ...
    top_winners = (
        ...
        .filter(Fighter.ufcstats_id.isnot(None))
        .order_by(Fighter.wins.desc())
        .limit(10)
        .all()
    )
    ...
    top_defense = (
        session.query(Fighter)
        .filter(Fighter.str_def.isnot(None), Fighter.ufcstats_id.isnot(None))
        .order_by(Fighter.str_def.desc())
        .limit(10)
        .all()
    )
    ...
    for i, fighter in enumerate(top_defense, 1):
        print(
            f"{i:2}. {fighter.name:30} | Str Def: {fighter.str_def:5.1f}% | TD Def: {fighter.td_def or 0:5.1f}%"
        )
    ...
    top_events = (
        ...
        .join(Fight)
        .filter(Event.ufcstats_id.isnot(None))
        .group_by(Event.id)
        .order_by(func.count(Fight.id).desc())
        .limit(10)
        .all()
    )
    ...
    top_kd_fights = (
        ...
        .order_by((Fight.r_kd + Fight.b_kd).desc())
        .limit(10)
        .all()
    )
    ...
    total_fighters = (
        session.query(func.count(Fighter.id))
        .filter(Fighter.ufcstats_id.isnot(None))
        .scalar()
    )
    ...
    total_fights = (
        ...
        .filter(Fight.ufcstats_id.isnot(None))
        .scalar()
    )

    avg_slpm = (
        session.query(func.avg(Fighter.slpm)).filter(Fighter.slpm.isnot(None)).scalar()
    )

    avg_str_acc = (
        session.query(func.avg(Fighter.str_acc))
        .filter(Fighter.str_acc.isnot(None))
        .scalar()
    )
    ...
    famous_fighter = (
        session.query(Fighter).filter(Fighter.name.ilike("%conor%mcgregor%")).first()
    )

    if famous_fighter:
        print(f"\nLutador: {famous_fighter.name}")
        print(
            f"Record: {famous_fighter.wins}-{famous_fighter.losses}-{famous_fighter.draws}"
        )
        print(f"Stance: {famous_fighter.stance or 'N/A'}")
        print(
            f"Height: {famous_fighter.height_cm:.0f}cm"
            if famous_fighter.height_cm
            else "Height: N/A"
        )
        print(
            f"Reach: {famous_fighter.reach_cm:.0f}cm"
            if famous_fighter.reach_cm
            else "Reach: N/A"
        )
        print("\nÚltimas 5 lutas:")
        ...
        if famous_fighter.cartel:
            for i, fight in enumerate(famous_fighter.cartel[:5], 1):
                ...
                print(
                    f"  {i}. {result_emoji} vs {fight['opponent']} - {fight['method']} (R{fight.get('round', '?')})"
                )
